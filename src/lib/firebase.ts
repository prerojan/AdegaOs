...
// Environment variable reader for Vite / Vercel with multiple name fallbacks
const getEnv = (key: string): string => {
  const metaEnv = (import.meta as any)?.env || {};
  const procEnv = (typeof process !== 'undefined' && process.env) ? (process.env as any) : {};

  const cleanKey = key.replace(/^(VITE_|REACT_APP_|NEXT_PUBLIC_)/, '');
  const candidateKeys = [
    key,
    `VITE_${cleanKey}`,
    `REACT_APP_${cleanKey}`,
    `NEXT_PUBLIC_${cleanKey}`,
    cleanKey
  ];
...
// Fallback LocalStorage State Listeners (when Firebase is disabled or cross-tab local sync)
const listeners = {
...
// Global cross-window storage change listener for multi-tab/window sync
if (typeof window !== 'undefined') {
  window.addEventListener('storage', (e) => {
    if (!e.key || !e.newValue) return;
    try {
      const data = JSON.parse(e.newValue);
      if (e.key === 'fluxos_tables') listeners.tables.forEach(cb => cb(data));
      if (e.key === 'fluxos_products') listeners.products.forEach(cb => cb(data));
      if (e.key === 'fluxos_sales') listeners.sales.forEach(cb => cb(data));
      if (e.key === 'fluxos_transactions') listeners.transactions.forEach(cb => cb(data));
      if (e.key === 'fluxos_users') listeners.users.forEach(cb => cb(data));
    } catch (err) {
      console.warn('Error reading cross-tab storage event:', err);
    }
  });
}
  ...
  if (db) {
    return onSnapshot(collection(db, 'users'), (snapshot) => {
      if (snapshot.empty) {
        ...
        const uList = snapshot.docs.map(d => d.data() as CashierUser);
        setCollection('users', uList, listeners.users);
        callback(uList);
      }
    }, (err) => {
...
export async function fetchTransactionsFromDb(): Promise<FinancialTransaction[]> {
  if (db) {
    try {
      ...
      if (!snapshot.empty) {
        return snapshot.docs.map(d => d.data() as FinancialTransaction);
      }
    } catch (err) {
...
export async function fetchUsersFromDb(): Promise<CashierUser[]> {
  if (db) {
    try {
      ...
      if (!snapshot.empty) {
        return snapshot.docs.map(d => d.data() as CashierUser);
      }
    } catch (err) {
